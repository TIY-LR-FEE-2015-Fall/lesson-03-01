// Before a credit card is submitted to a financial institution,
// it generally makes sense to run some simple reality checks on the number.
// The numbers are a good length and it's common to make minor transcription
// errors when the card is not scanned directly.

// The first check people often do is to validate that the card matches
//  a known pattern from one of the accepted card providers.
...
function isValid(ccnum) {
  var list = ccnum.replace(/\s/g, '').split('');
  var sum = 0;
    ...
    if (i % 2 === 0) {
      ...
      list[i] = list[i].toString()
        ...
        .reduce(function(carry, item) {
          return parseInt(carry) + parseInt(item);
        }, 0);
    }
